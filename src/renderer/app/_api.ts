...
function deleteElement(dbStore: ElectronStore, key: DBElementKeys, id: number) {
          ...
          (total, receiptProduct) => {
            total.newFlavors = receiptProduct.flavors.reduce(
              (newFlavors, flavorId) => {
                ...
                const newFlavorSold = newFlavors.find(
                  (el) => el.id === flavorId
                );
                if (newFlavorSold) {
                  // NOTE: Kind of a side effect here
                  newFlavorSold.units =
                    newFlavorSold.units - receiptProduct.quantity;
                  return newFlavors;
                }
            ...
            if (newProductSold) {
              // NOTE: Kind of a side effect here
              newProductSold.units =
                newProductSold.units - receiptProduct.quantity;
              return total;
            }
...
export function getReportProducts(
  ...
  return Promise.all([getProducts(), getReceipts()]).then(
    ([products, receipts]) => {
      return products.map((product) => {
        ...
        return {
          ...
          totalPrice: reports.reduce(
            (total, el) => total + el.totalPriceSold,
            0
          ),
        };
      });
...
export function getReportCategories(
  ...
  return Promise.all([getProducts(), getCategories(), getReceipts()]).then(
    ([products, categories, receipts]) => {
...
export function getReportFlavors(
  ...
  return Promise.all([getFlavors(), getProducts(), getReceipts()]).then(
    ([flavors, products, receipts]) => {
...
export function getReportReceipts(filters: ReportsArgs['filters'] = {}) {
  return getReceipts().then((receipts) =>
    receipts.filter((receipt) => isBetweenDates(receipt.date, filters.date))
  );
}
