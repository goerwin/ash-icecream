...
import { StoreState, Category, Product, Flavor, ReportProduct, ReportsArgs } from '../_models'
import { setErrorMessage } from '../_actions';
...
export default (options: {
  ...
  <T extends {}>(Comp: (React.ComponentType<T & InjectedProps>)) => {
  class ReportPage extends React.Component<T & Props> {
    ...
    get totalElementsSold() {
      return this.state.reportElements.reduce((total, el) => total + el.totalQuantitySold, 0)
    }

    // TODO: This has to be in the child component
    get totalPriceSold() {
      return convertToPriceFormat(
        this.state.reportElements.reduce((total, el) => total + el.totalPrice, 0)
      )
    }

    get confirmationMessage() {
      const el = this.state.reportElements.find(el => this.state.elementIdRowSelected == el.id)
    ...
    operateReportElements(filters: ReportsArgs['filters'] = {}) {
      ...
      return options.getReportElements(filters)
      .then(reportElements => {
    ...
    handleEditClick = () => {
      if (!this.state.elementIdRowSelected) { return; }
      this.setState({ ...this.state, isDialogElementOpened: true, isElementEdit: true, })
    }
    ...
    handleRemoveElement = () => {
      if (!this.state.elementIdRowSelected) { return; }
      this.props.onDeleteElement(this.state.elementIdRowSelected)
        .then(() => this.operateReportElements())
        .then(this.closeConfirmationDialog)
    }

    handleDeleteAllElements = () => {
      options.deleteAllElementsFn && options.deleteAllElementsFn()
        .then(() => this.operateReportElements())
        .then(this.closeConfirmationDialog)
    }
    ...
    closeConfirmationDialog = () => {
      this.setState({ ...this.state, showConfirmDelete: false, showConfirmDeleteAll: false })
    }
  ...
  const mapDispatchToProps = (dispatch: any) => ({
    onCloseNotification: () => { dispatch(setErrorMessage('')) },
    onSaveElement: (element: any) => {
      if (!options.saveElementThunk) { return }
  ...
  return connect(
      mapStateToProps,
      mapDispatchToProps
    // TODO: have to put any here so withRouter doesnt yell at me :(
    )(withRouter<any>(ReportPage))
}
