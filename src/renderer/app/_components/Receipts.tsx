...
class Receipts extends React.Component<Props, IState> {
  ...
  get totalPrice() {
    ...
    return convertToPriceFormat(receipts.reduce((total, el) => total + el.total, 0))
  }

  get totalElementsSold() {
    const receipts = this.props.reportElements as Receipt[]
    return receipts.reduce((total, el) =>
      total + el.products.reduce((total2, el2) => total2 + el2.quantity, 0)
    , 0)
  }

  handleRowDoubleClick = (idx: number, id: string) => {
    const receipt = (this.props.reportElements as Receipt[]).find(el => el.id === Number(id))
        ...
        <Reporter
          ...
          elements={(this.props.reportElements as Receipt[]).map(el => ({
            ...
            elements: [
              ...
              String(el.products.reduce((total, el2) => total + el2.quantity, 0)),
          ...
          buttons={[
            ...
            { id: 'removeAll', onClick: this.props.onDeleteAllElementsClick, name: '🗑 Todos' },
          ]}
        />
      </>
    )
